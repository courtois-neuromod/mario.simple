...
recordings start on the black ``WORLD x-y'' screen, so the level palette is written about 120
frames later; 143 (79 of them the first repetition of a run) start in gameplay and would keep the
original colours until the next area change. A companion tool rewrites the 32 palette bytes inside
the initial state; the run is RAM-identical afterwards. Second, where a collision box sticks out of
