...
into game logic.  A savestate taken mid-replay on
...
def main():
    ...
    for bk2 in replays:
        ...
        mask = np.ones(ro.shape[1], bool); mask[PRESENTATION] = False
        diff = ro != rm
        ...
        print(f"{os.path.basename(bk2)}: frames={len(keys)} differing_addresses={rng} "
              f"frames_differing_outside_ppu_buffers={outside} state_tracks={first_bad is None} -> {'OK' if good else 'FAIL'}")
